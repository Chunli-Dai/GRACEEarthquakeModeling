...
!!Modified from GNR_STATC_fast_JP_var_woset0.f , dcl, Feb 4 ,2013
!!      Purpose:  add grid file to parm
!!Modified from GNR_STATC_fast_JP_var_woset0_bw61.f, Feb 5, 2013
!!      Purpose: add nmax to parm
!!Modified from GNR_STATC_fast_JP_var_woset0_gdf_nmax.f, dcl, June 6, 2014
...
!       open(unit=1,file="grid_JP",STATUS="OLD")
...
!       'Prepare for gra and grad...'
        do i=1,npt
           ...
           lat(i)=90d0-lat(i);
        enddo
        ALLOCATE(APgrad(6,NCOEFS,npt))
        call subAPgrad(APgrad,NMAX,npt,LAT,LON,R)
        do i=1,npt
           lat(i)=90d0-lat(i); lat(i)=lat(i)*D2R; lon(i)=lon(i)*D2R
        enddo
...
CC      Matrix AP(9,NCOEFS)
        ALLOCATE(AP(9,NCOEFS))
!	include "AP.f"

CC      ASRP(4,NCOEFS)=ASR*AP
CC        ALLOCATE(ASRP(4,NCOEFS),ASRPT(NCOEFS,4))
	ALLOCATE(ASRP(6,NCOEFS),ASRPT(NCOEFS,6))
cc	ALLOCATE(ATPAT(NCOEFS,NCOEFS))
CC        ASRP=matmul(ASR,AP)
!!      AP: xx,xy,xz,yx,yy,yz,zx,zy,zz
!!      ASRP: XX,yy,zz,xy,xz,yz 
!	ASRP(1,:)=AP(1,:);ASRP(2,:)=AP(5,:)
!	ASRP(3,:)=AP(9,:);ASRP(4,:)=AP(2,:)
!	ASRP(5,:)=AP(3,:);ASRP(6,:)=AP(6,:)
        ASRP=APgrad(:,:,I)
        call BRMUL(ASRP,EST,6,NCOEFS,1,gradxyz)
!       call BRMUL(ASRP,DISP,6,NCOEFS,NCOEFS,AMP)
!       call BRMUL(AMP,ASRPT,6,NCOEFS,6,covgrad)
        covgrad=0d0
        do j=1,6
        do k=1,NCOEFS
          covgrad(j)=covgrad(j)+ASRP(j,k)**2*DISP(k)**2
        enddo
        enddo

        write(2,'(2F10.4,12(E17.8E2))')90d0-lat(i)/D2R,lon(i)/D2R
     +          ,gradxyz(1:6)*1d12,dsqrt(covgrad(1:6))*1d12

CC	DEALLOCATE(AP,ASRP,ASRPT,ASR)
	DEALLOCATE(AP,ASRP,ASRPT)	
